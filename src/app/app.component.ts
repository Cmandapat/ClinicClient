...
export class AppComponent {
  title = 'ClinicClient';
  user:UserCred;
  constructor(private router: Router, private activeRouter: ActivatedRoute, private location:Location, private userLoginService:UserLoginService){}
  ...
  isAdminNotHome(): boolean{
    ...
    || (this.location.path().indexOf('/doctoredit') > -1) || (this.location.path().indexOf('/appointment1') > -1)
    || (this.location.path().indexOf('/appointment') > -1) || (this.location.path().indexOf('/appointment2') > -1)
    ) && (!(this.location.path().indexOf('/admin') > -1)) && (this.userLoginService.adminLoggedIn)){
      return true;
    } else 
      return false;
  }
  ...
  doShowAppointment(): boolean{
    if(this.areLoginOrCreateActive() || this.isNoRouteActivated()){
      return false;
    } else if ((this.location.path().indexOf('/appointment1') > -1) || (this.location.path().indexOf('/reporter') > -1)){
      return false;
    } else {
      return true;
    }
